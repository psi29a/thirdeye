...
- `build/` — Debug -O0. Use for VM correctness work (gdb-friendly, asserts on).
- `build-release/` — Release. **5–10× faster interpreter**, use for anything that requires
  running long enough to observe behavior (combat, AI, perf). Configure with
  `cmake -DCMAKE_BUILD_TYPE=Release -S . -B build-release`.

If you just edited engine code, build the one you're about to run:
```sh
...
| `THIRDEYE_TESTMON=1` | Trace SEND dispatch for combat-relevant messages (163/77/78/71/162/43/82/55/22/235 + AI 91/85/107/99 on monster-range objects). Format: `[mon-msg] idx N msg M handler C arg0=… arg1=…`. Capped at 600 lines — bump in [vm/objects.cpp:301](../../../apps/thirdeye/vm/objects.cpp#L301) if you need more. |
...
```sh
THIRDEYE_TESTMON=1 THIRDEYE_ATTACK=1 THIRDEYE_MONCLASS=1934 \
  build-release/thirdeye.app/Contents/MacOS/thirdeye ../data/EYE.RES \
...
## Common gotcha: `THIRDEYE_TESTMON` cap
