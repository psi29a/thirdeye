...
- Write pipeline calls three writers in order (identities pinned via
  ...
  - `FUN_1325_3bb0` → **`items.dat`** (DOS-collapsed to same file
    the SOP opens as `ITEMS.DAT`) — 5-byte records with byte
    permutation `[3,1,0,2,4]`, zero-record terminator
  - `FUN_1325_3c76` → **`FEA%02d.DAT`** (one file per level) —
    8-byte records; loop has a 10-case switch on record type
