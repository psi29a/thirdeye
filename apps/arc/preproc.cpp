...
//��  PREPROC.C                                                             ��
//��                                                                        ��
//��  Preprocessor class for AESOP ARC compiler                             ��
//��                                                                        ��
//��  Version: 1.00 of 26-Feb-92 -- Initial version                         ��
//��                                                                        ��
...
BYTE *PD_keywords[] = { PD_DEFINE, PD_UNDEF, PD_INCLUDE, PD_DEPEND, PD_IFDEF,
		PD_IFNDEF, PD_ELSE, PD_ENDIF, PD_VERBOSE, PD_BRIEF };
...
void PP_process(PP_class *PP) {
	...
	inbuf = PP->inbuf;
	outbuf = PP->outbuf;         // (aliased for speed)
	...
	while (!done) {
		...
		for (i = 0; i < llen - nt; i++) {
			...
			if ((!qpsl) && (!qpcc) && (!qptl) && (!esc)) { // maintain comment flags
				if ((!com_blk) && (!com_lin))
					if ((chr == '/') && (chrnxt == '/')) {
						i = i + 1;
						com_lin = 1;
				...
				if ((chr == '*') && (chrnxt == '/')) {
					if (!com_blk)
						report(E_ERROR, TF_line_info(&PP->cur), MSG_UBC,
								PP->cur.name);
					else
						--com_blk;
					i = i + 1;
			...
			if ((chr == '\\') && (!esc))              // concatenate lines
				...
				if (j == llen) {
					...
					stat = PP_fetch_line(PP);
					if (stat == 1) {
						report(E_ERROR, TF_line_info(&PP->cur), MSG_CCL,
								PP->cur.name);
						return;
					} else if (stat == 2)
						break;
			...
			if (!esc)                                 // maintain quote parity
				if ((chr == '\'') && (!(qpsl || qptl)))
					...
					if (!qptl)
						report(E_ERROR, TF_line_info(&PP->cur), MSG_UBB,
								PP->cur.name);
					else
						qptl = 0;
			...
			if (name_end == -1)
				continue;             // (not end of identifier)
		...
		if (cur_mac != NULL) {                                // #defining macro
			while (is_whitespace[outbuf[mac_out]])
				mac_out++;
		...
		{                                         // descriptor to output
			for (j = strlen(outbuf); j; j--)              // file
				if (!is_whitespace[outbuf[j - 1]]) { // kill trailing whitespace
					outbuf[j] = 0;
					break;
				}
