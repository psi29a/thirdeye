...
BYTECODEPOINTER *readBytecodeDefinition(void)
{
    ...
    strcpy(loDefinitionFilePath, myHomeDirectory);
    ...
    {
        printf("Failure to allocate %d bytes for the bytecode definition table!", loBytecodeTableSize);
...
int processBytecodeDefinitionLine(char *aLine)
{
    ...
    */
    ...
    {
        loBytecodeEntryPointer->paramString = (char*)malloc(sizeof(char) * loNumberOfParameters + 1);
    }
    ...
    // explanation
    loBytecodeEntryPointer->explanation = makeString(loTokens[3 + loNumberOfParameters]);
    if (loBytecodeEntryPointer->explanation == NULL || strlen(loBytecodeEntryPointer->explanation) == 0)
    {
    ...
    FILE *aOutputFile, FILE *aResFile, DIRPOINTER *aDirectoryPointers)
{
    ...
    {
        printf("The disassembly cannot be done because the export resource is not available!\n");
        return;
    }
    ...
    {
        EXPORTENTRYPOINTER loCurrentExportEntry = aFullExportResourceDictionary[i];
        ...
        else if (loCurrentExportEntry->exportType == EXPORT_ENTRY_MESSAGE_HANDLER)
        {
            // it is a message handler
            int loMessageHandlerStart = loCurrentExportEntry->messageHandlerStart;
            if (loMessageHandlerStart != -1)
            {
                // there is a valid start address
                // mark it in the code map address as a disasembly start
                printf("Message handler \"%s\" start address: %d\n", loCurrentExportEntry->messageHandlerName, loMessageHandlerStart);
                setCodeMapAddress(loMessageHandlerStart, MAP_MESSAGE_HANDLER_NOT_DONE);                 
            }
        }
    }
    ...
    // meta information
    fprintf(aOutputFile, "\n%s %d\n", META_DISASSEMBLER_VERSION, DISASSEMBLER_VERSION);
    fprintf(aOutputFile, ";\n");
    ...
    if (loScriptHeader.parent != 0xffffffff)
    {
        ...
        fprintf(aOutputFile, "%s \"%s\"\n", META_PARENT_RESOURCE_NAME, loParentName);          
    }   
    fprintf(aOutputFile, ";\n");
    fprintf(aOutputFile, ";other original settings\n");
    fprintf(aOutputFile, "%s #%08x ;%ld\n", META_ORIGINAL_PARENT_RESOURCE_NUMBER,
        loScriptHeader.parent, (long)loScriptHeader.parent);            
    fprintf(aOutputFile, "%s %d\n", META_ORIGINAL_CODE_RESOURCE_SIZE, aLength);
    fprintf(aOutputFile, "%s #%04x     ;%d\n", META_ORIGINAL_CODE_RESOURCE_NUMBER,
        (int)aCodeResourceNumber, (int)aCodeResourceNumber);     
    fprintf(aOutputFile, "%s %d\n", META_ORIGINAL_IMPORT_RESOURCE_SIZE, aImportResourceSize);
    fprintf(aOutputFile, "%s #%04x     ;%d\n", META_ORIGINAL_IMPORT_RESOURCE_NUMBER,
        (int)loScriptHeader.import_resource, (int)loScriptHeader.import_resource);    
    fprintf(aOutputFile, "%s %d\n", META_ORIGINAL_EXPORT_RESOURCE_SIZE, aExportResourceSize);
    fprintf(aOutputFile, "%s #%04x     ;%d\n", META_ORIGINAL_EXPORT_RESOURCE_NUMBER,
        (int)loScriptHeader.export_resource, (int)loScriptHeader.export_resource);    
    fprintf(aOutputFile, "%s %d\n", META_ORIGINAL_STATIC_SIZE, (int)loScriptHeader.static_size);
    ...
    if (loResourcesInfoTable == NULL)
    {
        ...
        const char *loError = "Error: unable to get the resource information table!";
        printf("%s\n", loError);
        fprintf(aOutputFile, "%s\n", loError);
        return;
    }
          
    printf("Starting the second disassembly pass for the code resource: %s ...\n", loObjectName);
    
    // second pass (writes output)
    makeSecondDisassemblyPass(aResource, aLength, aFullImportResourceDictionary, aFullExportResourceDictionary, aOutputFile,
            aResFile, aDirectoryPointers, loResourcesInfoTable);
...
*/
int makeFirstDisassemblyPass(unsigned char *aResource, int aLength, int aStartAddress, IMPORTENTRYPOINTER *aFullImportResourceDictionary)
{
    ...
    for(;;)
    {
        ...
        loInstructionLength = getInstructionLength(aResource, aLength, loCurrentAddress);
        ...
        handleVariableAndArrayRelatedInstructions(aResource, aLength, loCurrentAddress, aFullImportResourceDictionary);
       
        // mark the instruction bytes as disassembled
        for(i = loCurrentAddress; i < loCurrentAddress + loInstructionLength; i++)
        {
...
int getInstructionLength(unsigned char *aResource, int aResourceLength, int aCurrentAddress)
{
    ...
    {
        printf("getInstructionLength(): there is no instruction with the code %x on the address %d!\n", loInstruction, aCurrentAddress);
        return -1;            
    }
    ...
    if (loParamCount == PARAMETERS_HANDLED_BY_CODE)
    {
        if (loInstruction ==INSTRUCTION_CASE)
        {
            ...
            {
                printf("The CASE instruction parameters are outside of the code resource!\n");
                return -1;
            }            
            return loLength;           
        }
        ...
        {
            // looks like nosense
            printf("The parameter handling in code is not supported for the instruction %s on the address %d!\n",
    ...
    for(i = 0; i < loParamCount; i++)
    {
         ...
         else if (loParameterType == 'w' || loParameterType == 'i' || loParameterType == 'm')
         {
             loLength += 2;
         }
    ...
    if (aCurrentAddress + loLength > aResourceLength)
    {
        printf("The instruction parameters are outside of the code resource!\n");
        return -1;
    }         
    return loLength;    
}
...
void setTargetsForTheInstruction(unsigned char *aResource, int aLength, int aOriginalAddress)
{
    ...
    if (loInstruction > MAX_BYTECODES || bytecodeTable[loInstruction] == NULL)
    {
        printf("setTargetsForTheInstruction(): there is no instruction with the code %x on the address %d!\n", loInstruction, loCurrentAddress);
        return;            
    }
    loCurrentAddress++;
...
char *getRuntimeCodeFunctionName(char *aResult, long aFunctionNumber, IMPORTENTRYPOINTER *aFullImportResourceDictionary)
{
    ...
    {
        if (aFullImportResourceDictionary[i]->importType == IMPORT_ENTRY_RUNTIME_FUNCTION)
        {
            if (aFunctionNumber == aFullImportResourceDictionary[i]->runtimeFunctionNumber)
            {
                strcpy(aResult, aFullImportResourceDictionary[i]->firstOriginal);
                return aResult;
            }
        }
    }
        ...
        DIRPOINTER *aDirectoryPointers, RESINFOPOINTER *aResourcesInfoTable)
{
    ...
    for(loCurrentAddress = 14; loCurrentAddress < aLength;)
    {
        ...
        if (loCodeMapValue == MAP_MESSAGE_HANDLER_PROCESSED)
        {
            ...
            fprintf(aOutputFile, ";\n");
            fprintf(aOutputFile, "%s \"%s\"\n", META_MESSAGE_HANDLER_NAME, loMessageHandlerName);
            // first two bytes of a message handler are local variables
            memcpy(&loLocalVariablesSize, aResource + loCurrentAddress, 2);
            fprintf(aOutputFile, "%s %d    ;includes 2 bytes for THIS pointer\n", META_LOCAL_VARIABLES_SIZE, loLocalVariablesSize);
            loEndAddress = getAddressBehindTheFunction(loCurrentAddress, aLength);
        ...
        else if (loCodeMapValue == MAP_PROCEDURE_START_PROCESSED)
        {
            ...
            fprintf(aOutputFile, ";\n");
            fprintf(aOutputFile, "%s \"%s\"\n", META_PROCEDURE_NAME, loProcedureName);
        ...
        else if (loCodeMapValue == MAP_BYTE_TABLE_START)
        {
            ...
            fprintf(aOutputFile, "%s \"%s\" ;%d\n", META_TABLE_BYTE, loTableName, loCurrentAddress);
            getDBByteString(loDBString, aResource, aLength, loCurrentAddress);
            fprintf(aOutputFile, "%s\n", loDBString);
            loCurrentAddress++;            
        }
        else if (loCodeMapValue == MAP_WORD_TABLE_START)
        {
            ...
            fprintf(aOutputFile, "%s \"%s\" ;%d\n", META_TABLE_WORD, loTableName, loCurrentAddress);
            getDWWordString(loDBString, aResource, aLength, loCurrentAddress);
            fprintf(aOutputFile, "%s\n", loDBString);
            loCurrentAddress+=2;            
        }
        else if (loCodeMapValue == MAP_LONG_TABLE_START)
        {
            ...
            fprintf(aOutputFile, "%s \"%s\" ;%d\n", META_TABLE_LONG, loTableName, loCurrentAddress);
            getDLLongString(loDBString, aResource, aLength, loCurrentAddress);
            fprintf(aOutputFile, "%s\n", loDBString);
            loCurrentAddress+=4;            
        }               
        ...
        {
           fprintf(aOutputFile, "Error in the code table on the address %d with the value %d (%02x hexadecimal)!\n",
                loCurrentAddress, (int)(unsigned char)aResource[loCurrentAddress],
        ...
        {
            char loError[256];
            sprintf(loError, "Unimplemented code map value %d on the address %d\n", loCodeMapValue, loCurrentAddress);
            printf("%s\n", loError);
            fprintf(aOutputFile, "%s\n", loError);
            return;
        }
...
char *getMessageHandlerNameForAddress(int aEntryPointAddress, EXPORTENTRYPOINTER *aFullExportResourceDictionary)
{
    ...
    for(i = 0; aFullExportResourceDictionary[i] != NULL; i++)
    {
        ...
        if (loCurrentExportEntry->exportType == EXPORT_ENTRY_MESSAGE_HANDLER)
        {
            ...
            if (aEntryPointAddress == loCurrentExportEntry->messageHandlerStart)
            {
                // entry points agree
...
*/
void getDBByteString(char *aResult, unsigned char *aResource, int aLength, int aAddress)
{
    ...
    {
        strcpy(aResult, "getDBBytestring() is called for the address outside of the code resource!\n");
        return;
    }
...
*/
void getDWWordString(char *aResult, unsigned char *aResource, int aLength, int aAddress)
{
    char loResultLine[256];
    char loTmp[256];
    char loHexaCodes[256];
    ...
    if (aAddress < 0 || aAddress >= aLength)
    {
        strcpy(aResult, "getDWWordstring() is called for the address outside of the code resource!\n");
        return;
    }
...
*/
void getDLLongString(char *aResult, unsigned char *aResource, int aLength, int aAddress)
{
    char loResultLine[256];
    char loTmp[256];
    char loHexaCodes[256];
    ...
    if (aAddress < 0 || aAddress >= aLength)
    {
        strcpy(aResult, "getDLLongstring() is called for the address outside of the code resource!\n");
        return;
    }
        ...
        FILE *aOutputFile, FILE *aResFile, DIRPOINTER *aDirectoryPointers, RESINFOPOINTER *aResourcesInfoTable)
{
    ...
    if (loCurrentInstruction > MAX_BYTECODES || bytecodeTable[loCurrentInstruction] == NULL)
    {
        char loError[256];
        sprintf(loError, "writeOneInstruction(): there is no instruction with the code %x on the address %d!\n",
    ...
    (*loCurrentAddress)++;
    // instruction name
    sprintf(loTmp, INSTRUCTION_FORMAT_STRING, bytecodeTable[loCurrentInstruction]->name);
    ...
    if (loParamCount == PARAMETERS_HANDLED_BY_CODE)
    {
        ...
        {            
            // looks like nonsense
            char loError[256];
            sprintf(loError, "The parameter handling in code is not supported for the instruction %s on the address %d!",
        ...
        for(i = 0; i < loParamCount; i++)
        {
            ...
            if (loParameterType == 'i')
            {
                ...
                if (loCurrentInstruction == INSTRUCTION_RCRS)
                {
                    ...
                    if (getRuntimeCodeFunctionName(loRuntimeCodeResourceName, loParameterValue,
                            aFullImportResourceDictionary) == NULL)
                    {
                        strcpy(loRuntimeCodeResourceName, "__unknown_runtime_function__");
                    }
                ...
                else if (loCurrentInstruction == INSTRUCTION_LXB || loCurrentInstruction == INSTRUCTION_LXW ||
                        loCurrentInstruction == INSTRUCTION_LXD || loCurrentInstruction == INSTRUCTION_SXB ||
                        loCurrentInstruction == INSTRUCTION_SXW || loCurrentInstruction == INSTRUCTION_SXD ||
                        loCurrentInstruction == INSTRUCTION_LEXA)  // Warning: just assume that LEXA refers a normal variable (not array)
                {
                    ...
                    if (getExternalVariableNameForVariableNumber(loVariableName, loParameterValue,
                            aFullImportResourceDictionary) == NULL)
                    {
                ...
                else if (loCurrentInstruction == INSTRUCTION_LXBA || loCurrentInstruction == INSTRUCTION_LXWA ||
                        loCurrentInstruction == INSTRUCTION_LXDA || loCurrentInstruction == INSTRUCTION_SXBA ||
                        loCurrentInstruction == INSTRUCTION_SXWA || loCurrentInstruction == INSTRUCTION_SXDA)
                {
                    char loArrayName[256];
                    if (getExternalVariableNameForVariableNumber(loArrayName, loParameterValue,
                            aFullImportResourceDictionary) == NULL)
                    {
            ...
            else if (loParameterType == 'm')
            {
                ...
                if (getMessageName(loMessageName, loMessageID, aResFile, aDirectoryPointers) == NULL)
                {
            ...
            {
                if ((loCurrentInstruction == INSTRUCTION_BRT || loCurrentInstruction == INSTRUCTION_BRF ||
                        loCurrentInstruction == INSTRUCTION_BRA ||
                        loCurrentInstruction == INSTRUCTION_LECA) &&
                        loParamCount == 1)
                {
                    // show label for BRT/BRF/BRA/LECA
                    sprintf(loTmp, "%s%d ;%d", LABEL_PREFIX, (int)loParameterValue, (int)loParameterValue);
                    strcat(loInstruction, loTmp);                    
                    
                }
                else if (loCurrentInstruction ==INSTRUCTION_LSB || loCurrentInstruction ==INSTRUCTION_LSW ||
                        loCurrentInstruction ==INSTRUCTION_LSD || loCurrentInstruction ==INSTRUCTION_SSB ||
                        loCurrentInstruction ==INSTRUCTION_SSW || loCurrentInstruction ==INSTRUCTION_SSD ||
                        loCurrentInstruction ==INSTRUCTION_LSBA || loCurrentInstruction ==INSTRUCTION_LSWA ||
                        loCurrentInstruction ==INSTRUCTION_LSDA || loCurrentInstruction ==INSTRUCTION_SSBA ||
                        loCurrentInstruction ==INSTRUCTION_SSWA || loCurrentInstruction ==INSTRUCTION_SSDA ||
                        loCurrentInstruction ==INSTRUCTION_LESA)
                {
                    ...
                    if (getStaticVariableNameForIndex(loStaticVariableName, loParameterValue) == NULL)
                    {
                        printf("Failure to get the name of the static variable %ld from the static variable table!\n",
                            loParameterValue);
                        strcpy(loStaticVariableName, "__unknown_static_variable__");                           
                    }
                    sprintf(loTmp, "\"%s\" ;%d", loStaticVariableName, (int)loParameterValue);
                    strcat(loInstruction, loTmp);                    
                }
                else if (loCurrentInstruction == INSTRUCTION_LAB || loCurrentInstruction == INSTRUCTION_LAW ||
                        loCurrentInstruction == INSTRUCTION_LAD || loCurrentInstruction == INSTRUCTION_SAB ||
                        loCurrentInstruction == INSTRUCTION_SAW || loCurrentInstruction == INSTRUCTION_SAD ||
                        loCurrentInstruction == INSTRUCTION_LABA || loCurrentInstruction == INSTRUCTION_LAWA ||
                        loCurrentInstruction == INSTRUCTION_LADA || loCurrentInstruction == INSTRUCTION_SABA ||
                        loCurrentInstruction == INSTRUCTION_SAWA || loCurrentInstruction == INSTRUCTION_SADA ||
                        loCurrentInstruction == INSTRUCTION_LEAA)
                {
                    // instructions working with local (auto) variables
                    char loAutoVariableName[256];
                    getAutoVariableNameForIndex(loAutoVariableName, loCurrentInstruction, loParameterValue);
                    sprintf(loTmp, "\"%s\" ;%d", loAutoVariableName, (int)loParameterValue);
                    strcat(loInstruction, loTmp);                    
                }
                else if (loCurrentInstruction ==INSTRUCTION_LTBA || loCurrentInstruction ==INSTRUCTION_LTWA ||
                        loCurrentInstruction ==INSTRUCTION_LTDA || loCurrentInstruction ==INSTRUCTION_LETA)
                {
                    // instructions working with constant tables
                    char loConstantTableName[256];
                    if (getConstantTableName(loConstantTableName, loParameterValue) == NULL)
                    {
                        printf("Failure to get the name of the constant table %ld!\n",
                            loParameterValue);
                        strcpy(loConstantTableName, "__unknown_constant_table__");                           
                    }
                    sprintf(loTmp, "\"%s\" ;%d", loConstantTableName, (int)loParameterValue);
                    strcat(loInstruction, loTmp);                    
                }                                         
                else if (loCurrentInstruction == INSTRUCTION_JSR)
                {
                    // show procedure name for JSR
                    sprintf(loTmp, "\"%s%d\" ;%d", PROCEDURE_PREFIX, (int)loParameterValue, (int)loParameterValue);
                    strcat(loInstruction, loTmp);                    
                ...
                else
                {
                    ...
                    // it is a number
                    if (loCurrentInstruction == INSTRUCTION_SHTC || loCurrentInstruction == INSTRUCTION_INTC ||
...
void getHexCodes(char *aString, unsigned char *aResource, int aBufferLength, int aStartAddress, int aSize)
{
    ...
    {
        // error
        strcpy(aString, "getHexCodes() called on addresses outside of the code resource!\n");
        return;
    }
...
void writeCaseEntry(unsigned char *aResource, int aLength, int aStartAddress, long aValue, int aTarget, FILE *aOutputFile)
{
     ...
     strcpy(loInstruction, loTmp);
     sprintf(loTmp, "#%08lx, %s%d     ;%ld, %d", aValue, LABEL_PREFIX, aTarget, aValue, aTarget);
     strcat(loInstruction, loTmp);
     // result
     strcpy(loResultLine, loHexCodes);
     strcat(loResultLine, loInstruction);
        ...
        IMPORTENTRYPOINTER *aFullImportResourceDictionary)
{
    ...
    if (loInstruction > MAX_BYTECODES || bytecodeTable[loInstruction] == NULL)
    {
        printf("markParameterAsArrayIfNeeded(): there is no instruction with the code %x on the address %d!\n", loInstruction, loCurrentAddress);
        return;            
    }
    loCurrentAddress++;
    switch(loInstruction)
...
char getUppercaseVariableType(int aInstruction)
{
    ...
    switch(aInstruction)
    {
        ...
        default:
            loResult = '?';
            printf("getUppercaseVariableType(): unknown variable type for the instruction %d\n", aInstruction);
            break;
...
int getAddressBehindTheFunction(int aStartAddress, int aLength)
{
    ...
    for(i = aStartAddress; i < aLength; i++)
    {
        ...
        if (loCodeMapValue ==MAP_MESSAGE_HANDLER_PROCESSED || loCodeMapValue == MAP_PROCEDURE_START_PROCESSED)
        {
    ...
    FILE *aOutputFile)
{
    ...
    {
        printf("writeInfoAboutReferredResourceIfAvailable(...): aResourcesInfoTable is NULL!\n");
    }
    ...
    for(i = 0; aResourcesInfoTable[i] != NULL ; i++)    
    {
        ...
        {            
            ...
            if (loResourceType == RESOURCE_TYPE_IMPORT || loResourceType == RESOURCE_TYPE_EXPORT)
            {
            ...
            fprintf(aOutputFile, "%7s%s \"%s\" (%s)\n", " ", ";possible reference to the resource ",
                aResourcesInfoTable[i]->name,
                loTypeString);
            if (aResourcesInfoTable[i]->resourceType == RESOURCE_TYPE_STRING && aResourcesInfoTable[i]->stringValue != NULL)
            {
                // write string value
