...
long getResourceEntryIndex(int aNumber, DIRPOINTER * aDirectoryPointers) {
	...
	if (loDirBlockNumber >= MAX_DIRECTORIES
			|| aDirectoryPointers[loDirBlockNumber] == NULL) {
		printf(
	...
	loDirItemIndex =
			aDirectoryPointers[loDirBlockNumber]->entry_header_index[loNumberInDirBlock];
...
unsigned char *readResourceBinary(int aResourceNumber, FILE *aResFile,
	...
	loResourceEntryIndex = getResourceEntryIndex(aResourceNumber,
			aDirectoryPointers);
	// length
