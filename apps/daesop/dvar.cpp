...
int initializeStaticVariableList(
	...
	loStaticVariablesTableSizeInBytes = sizeof(struct STATIC_VARIABLE_ENTRY)
			* (MAX_STATIC_VARIABLES + 1);
	myStaticVariablesTable = (STATVARPOINTER *) malloc(
	...
	if (aFullExportResourceDictionary == NULL) {
		...
		for (i = 0;
			...
			case EXPORT_ENTRY_LONG:
				loStatVarEntry = (STATVARPOINTER) malloc(
				...
				loStatVarEntry->name = makeString(
						loCurrentExportEntry->firstOriginal);
				loStatVarEntry->variableIndex =
						loCurrentExportEntry->exportedVariablePosition;
				loStatVarEntry->numberOfElements = -1;
				// store into the table of static variables
				myStaticVariablesTable[loStaticVariablesTableIndex++] =
						loStatVarEntry;
				break;
			...
			case EXPORT_ENTRY_ARRAY_LONG:
				...
				loStatVarEntry->name = makeString(
						loCurrentExportEntry->firstOriginal);
				loStatVarEntry->variableIndex =
						loCurrentExportEntry->exportedArrayPosition;
				loStatVarEntry->numberOfElements =
						loCurrentExportEntry->exportedArraySizeInElements;
				// store into the table of static variables
				myStaticVariablesTable[loStaticVariablesTableIndex++] =
						loStatVarEntry;
				break;
...
int addPrivateStaticVariableIfNotExisting(int aVariableIndex, int aIsArray,
	...
	for (loStaticVariablesTableIndex = 0;
			loStaticVariablesTableIndex < MAX_STATIC_VARIABLES
					&& myStaticVariablesTable[loStaticVariablesTableIndex]
							!= NULL; loStaticVariablesTableIndex++) {
		...
		if (loStatVarEntry->variableIndex == aVariableIndex) {
			...
			if ((loTypeCharacterPointer = strstr(loName, "?:")) != NULL
					&& aVariableType != '?') {
				// fix the type
		...
		if (loIsPublic == true) {
			...
			if (loIsArray == true) {
				...
				fprintf(aOutputFile, "%s %-18s  %d, %d\n",
						...
						loCurrentEntry->numberOfElements);
			} else {
				// public variable
...
int initializeConstantTableList(void) {
	...
	loConstantTablesTableSizeInBytes = sizeof(struct TABLE_CONSTANT_ENTRY)
			* (MAX_CONSTANT_TABLES + 1);
	myConstantTables = (TABLECONSTPOINTER *) malloc(
...
char *getConstantTableName(char *aResult, int aTableIndex) {
	...
	for (loTableIndex = 0;
			loTableIndex < MAX_CONSTANT_TABLES
					&& myConstantTables[loTableIndex] != NULL; loTableIndex++) {
...
void processConstantTableList(void) {
	...
	if (myConstantTables == NULL) {
		printf(
				"processConstantTableList(): the table of constant tables is NULL!\n");
		return;
	}
	for (i = 0; i < MAX_CONSTANT_TABLES + 1 && myConstantTables[i] != NULL;
			i++) {
		TABLECONSTPOINTER loCurrentEntry;
		int loTableIndex;
		char loElementType;
		...
		if (loElementType == 'B' || loElementType == '?') // byte or unknown type
			...
			printf(
					"processConstantTableList(): unknown constant table type %c\n",
					loElementType);
		}
	}
...
void getAutoVariableNameForIndex(char *aResult, int aCurrentInstruction,
	...
	case INSTRUCTION_SAB:
		if (aVariableIndex >= LOCAL_VARIABLES_START_INDEX
				&& aVariableIndex <= LOCAL_VARIABLES_END_INDEX) {
...
int initializeLocalVariableReferencesList(void) {
	...
	loLocalVariableReferencesTableSizeInBytes =
			sizeof(struct LOCAL_VARIABLE_REFERENCE)
					* (MAX_LOCAL_VARIABLE_REFERENCES + 1);
	myLocalVariableReferences = (LOCALVARIABLEREFERENCEPOINTER *) malloc(
			loLocalVariableReferencesTableSizeInBytes);
...
int addAutoVariableReference(int aAddress, int aInstruction, int aVariableIndex,
	...
	loNewEntry->isLocalVariableOrArray =
			(aVariableIndex >= LOCAL_VARIABLES_START_INDEX
					&& aVariableIndex <= LOCAL_VARIABLES_END_INDEX) ?
					true : false;
 ...
 */
int compareLocaleVariableReferences(void const *aFirstItem,
		void const *aSecondItem) {
	LOCALVARIABLEREFERENCEPOINTER loFirstEntry =
			*(LOCALVARIABLEREFERENCEPOINTER *) aFirstItem;
	LOCALVARIABLEREFERENCEPOINTER loSecondEntry =
			*(LOCALVARIABLEREFERENCEPOINTER *) aSecondItem;
...
void displayLocalVariableReferencesList() {
	...
	if (myLocalVariableReferences == NULL) {
		printf(
				"dumpLocalVariableReferencesList(...): myLocalVariableReferences is NULL!\n");
		return;
	}
...
int getParametersOrLocalVariables(LOCALVARIABLEPOINTER aResultArray[],
	...
	int loItemCounter = 0;
	for (i = 0; i < myLocalVariableReferencesCounter; i++) {
		LOCALVARIABLEREFERENCEPOINTER loVariableReferenceEntry =
				myLocalVariableReferences[i];
		if (loVariableReferenceEntry->address < aStartAddress
				|| loVariableReferenceEntry->address > aEndAddress) {
		...
		if ((aGetLocalVariables == false
			...
			if (j == loItemCounter) {
				...
				getAutoVariableNameForIndex(loTmp,
						loVariableReferenceEntry->instruction,
...
void writeParameters(FILE *aOutputFile, int aCurrentAddress, int aEndAddress) {
	LOCALVARIABLEPOINTER loParameters[MAX_FUNCTION_PARAMETERS_OR_LOCAL_VARIABLES
			+ 1];
	int i;
	...
	if (getParametersOrLocalVariables(loParameters, false, aCurrentAddress,
			aEndAddress) == false) {
	...
	for (i = 0; loParameters[i] != NULL; i++) {
		if (loParameters[i]->isArray == true) {
			...
			// parameter variable
			fprintf(aOutputFile, "%s %-18s  %d\n", META_PARAMETER_VARIABLE,
					loParameters[i]->name, loParameters[i]->index);
		}
	}
...
void writeLocalVariables(FILE *aOutputFile, int aCurrentAddress,
		int aEndAddress) {
	LOCALVARIABLEPOINTER loLocalVariables[MAX_FUNCTION_PARAMETERS_OR_LOCAL_VARIABLES
			+ 1];
	int i;
	for (i = 0; i < MAX_FUNCTION_PARAMETERS_OR_LOCAL_VARIABLES + 1; i++) {
	...
	if (getParametersOrLocalVariables(loLocalVariables, true, aCurrentAddress,
			aEndAddress) == false) {
	...
	for (i = 0; loLocalVariables[i] != NULL; i++) {
		if (loLocalVariables[i]->isArray == true) {
			...
			// local variable
			fprintf(aOutputFile, "%s %-18s  %d\n", META_LOCAL_VARIABLE,
					loLocalVariables[i]->name, loLocalVariables[i]->index);
		}
	}
