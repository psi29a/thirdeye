...
RESOURCES::Resource::Resource(boost::filesystem::path &resourcePath):mResFile(resourcePath) {
    ...
    if (boost::filesystem::exists(mResFile) == false) // does resource exist
        throw std::runtime_error(mResFile.string() + " does not exist!");

    std::cout << "  Loading: " << mResFile;

    resourceFileSize = boost::filesystem::file_size(mResFile);  // how big is the file on disk?

    std::cout << " " << resourceFileSize << " bytes " << std::endl;

    std::ifstream fResource; // open our resource
