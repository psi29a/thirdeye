...
const char XMIDI::mt32asgm[128] = { 0,	// 0	Piano 1
		1,	// 1	Piano 2
		2,	// 2	Piano 3 (synth)
		4,	// 3	EPiano 1
		4,	// 4	EPiano 2
...
const char XMIDI::mt32asgs[256] = { 0, 0,	// 0	Piano 1
		1, 0,	// 1	Piano 2
		2, 0,	// 2	Piano 3 (synth)
		4, 0,	// 3	EPiano 1
		4, 0,	// 4	EPiano 2
...
int XMIDI::ConvertEvent(const int time, const unsigned char status,
...
int XMIDI::ConvertSystemMessage(const int time, const unsigned char status,
		DataSource *source) {
	int i = 0;
...
unsigned int XMIDI::ConvertListToMTrk(DataSource *dest, midi_event *mlist) {
	...
	for (event = mlist; event && !end; event = event->next) {
		...
		case 0xF:
			...
			// Never occur
		default:
			std::cerr << "Not supposed to see this" << std::endl;
			break;
		}
	}
...
int XMIDI::ExtractTracks(DataSource *source) {
	...
	if (!memcmp(buf, "FORM", 4)) {
		...
		if (!memcmp(buf, "XMID", 4)) {
			...
			// Not an XMID
			if (memcmp(buf, "CAT ", 4)) {
				std::cerr << "Not a recognised XMID (" << buf[0] << buf[1]
						<< buf[2] << buf[3] << ") should be (CAT )"
						<< std::endl;
				return (0);
			}
		...
		if (count != info.tracks) {
			std::cerr << "Error: unable to extract all (" << info.tracks
