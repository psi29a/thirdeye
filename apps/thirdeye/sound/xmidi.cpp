...
int XMIDI::ExtractTracks (DataSource *source)
{
	...
	if (!memcmp (buf, "FORM", 4))
	{
		...
		{
			std::cerr << "Error: unable to extract all (" << info.tracks << ") tracks specified from XMIDI. Only ("<< count << ")" << std::endl;
	...
	else if (!memcmp (buf, "MThd", 4))
	{
		...
		if (count != info.tracks)
		{
			std::cerr << "Error: unable to extract all (" << info.tracks << ") tracks specified from MIDI. Only ("<< count << ")" << std::endl;
