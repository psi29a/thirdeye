...
void GRAPHICS::Graphics::zoomIntoImage(const std::vector<uint8_t> &bmp) {
    mState = ZOOM_INTO;
    mCounter = 0;
    mBuffer = bmp;
 ...
 * blank image.
 */
void GRAPHICS::Graphics::loadMouse(const std::vector<uint8_t> &bitmap,
        uint16_t index) {
    Bitmap image(bitmap);
...
void GRAPHICS::Graphics::loadPalette(const std::vector<uint8_t> &basePal,
