...
// temp surface
SDL_Surface SCREEN_0 = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 200, 8, 0, 0, 0, 0);
...
static uint8_t *s_gfx_screen = NULL;

static bool s_video_initialized = false;
static bool s_video_lock = false;

static uint16_t s_keyBufferLatest = 0;

static uint16_t s_mousePosX = 0;
static uint16_t s_mousePosY = 0;
static bool s_mouseButtonLeft  = false;
static bool s_mouseButtonRight = false;

static uint16_t s_mouseMinX = 0;
static uint16_t s_mouseMaxX = 0;
static uint16_t s_mouseMinY = 0;
static uint16_t s_mouseMaxY = 0;

static uint8_t s_gfx_screen8[SCREEN_WIDTH * SCREEN_HEIGHT];

/* Partly copied from http://webster.cs.ucr.edu/AoA/DOS/pdf/apndxc.pdf */
static uint8_t s_SDL_keymap[] = {
 ...
 */
static void Video_Key_Callback(uint8_t key)
{
...
bool Video_Init(void)
{
	...
	//FIXME: s_gfx_surface = SDL_SetVideoMode(SCREEN_WIDTH * SCREEN_MAGNIFICATION, SCREEN_HEIGHT * SCREEN_MAGNIFICATION, 8, SDL_SWSURFACE);

	if (s_gfx_surface == NULL) {
		printf("Could not set resolution: %s\n", SDL_GetError());
	...
	s_gfx_screen = (uint8_t *)s_gfx_surface->pixels;
...
{
	uint8_t *data = GFX_Screen_Get_ByIndex(SCREEN_0);
	uint8_t *gfx1 = s_gfx_screen;
	uint8_t *gfx2;
	uint8_t value;
	int x, y;
...
static void Video_DrawScreen(void)
{
	uint8_t *data = SCREEN_0; //FIXMEGFX_Screen_Get_ByIndex(SCREEN_0);
	uint8_t *gfx1 = s_gfx_screen;
	uint8_t *gfx2;
	int x, y;
...
void Video_Tick(void)
{
	...
	//FIXME: if (g_fileOperation != 0) return;
	...
	//FIXME:if (memcmp(GFX_Screen_Get_ByIndex(SCREEN_0), s_gfx_screen8, SCREEN_WIDTH * SCREEN_HEIGHT) == 0) {
	if (memcmp(SCREEN_0), s_gfx_screen8, SCREEN_WIDTH * SCREEN_HEIGHT) == 0) {
		s_video_lock = false;
		return;
	}
	//memcpy(s_gfx_screen8, GFX_Screen_Get_ByIndex(SCREEN_0), SCREEN_WIDTH * SCREEN_HEIGHT);
	memcpy(s_gfx_screen8, SCREEN_0, SCREEN_WIDTH * SCREEN_HEIGHT);
...
void Video_SetPalette(void *palette, int from, int length)
{
	...
	s_video_lock = true;
	SDL_SetPaletteColors(s_gfx_surface->format->palette, sdlPalette->colors, from, length);
	s_video_lock = false;
}
